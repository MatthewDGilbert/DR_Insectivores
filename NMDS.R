...
ggplot(nmds_coords, aes(x = MDS1, y = MDS2, color = Cluster)) +
  ...
  theme_minimal() +
  geom_text_repel(aes(label = Species), nudge_x = 0.08)
...
overlap <- rbind(AMRE, PRAW, NOPA) #use only the data from these four species for the graph
overlap <- rbind(PAWA, YRWA, OVEN) #use only the data from these four species for the graph
...
ggplot(overlap, aes(y = Bird.Height, x = Canopy.height, color = Species)) +
         ...
         stat_ellipse (geom = "polygon", level=0.80, size=0.7, aes(fill=Species), alpha=0.2)
...
# Filter out BBTO and BWVI
filtered_data <- data[!data$Species %in% c("BBTO", "BWVI"), ]

# Update factor levels for Species
filtered_data$Species <- factor(filtered_data$Species, 
                                levels = c("OVEN", "PAWA", "YRWA", "AMRE", "PRAW", "BAWW", "NOPA", "CMWA"))

# Update color palette to exclude BBTO and BWVI
filtered_cols <- c("OVEN" = "#00BF7D", "PAWA" = "#00BF7D", "YRWA" = "#E76BF3", 
                   "AMRE" = "#00B0F6", "PRAW" = "#00B0F6", "NOPA" = "#00B0F6", 
                   "BAWW" = "#00B0F6", "CMWA" = "#F8766D")


# Create the plot
ggplot(filtered_data, aes(x = Species, y = X..Height.in.Canopy, fill = Species), alpha = 0.5) +
  ...
  labs(
    x = "\nSpecies",
    y = "Proportional Height Within Canopy\n"
  ) +
...
# Plot the data
ggplot(adjusted_pval_table, aes(Species1, Species2, fill = Significance)) +
